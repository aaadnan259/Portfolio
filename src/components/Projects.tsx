    ...
    return (
        <section id="projects" className="py-20 bg-surface/30">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {/* Featured Projects with Video or Highlighted Image - Full Width */}
                <div className="space-y-8 mb-8">
                    {projects.filter(p => p.video || p.imagePosition).map((project, index) => (
                        <motion.div
                            key={index}
                            initial={{ opacity: 0, y: 20 }}
                            whileInView={{ opacity: 1, y: 0 }}
                        ...
                        >
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
                                {/* Media Container */}
                                <div className={`relative overflow-hidden bg-surface/20 min-h-[300px] lg:min-h-full ${project.imagePosition === 'right' ? 'lg:order-2' : ''}`}>
                                    {project.video ? (
                                        <video
                                ...
                                {/* Content Container */}
                                <div className={`p-8 flex flex-col justify-center ${project.imagePosition === 'right' ? 'lg:order-1' : ''}`}>
